...
const Specialprices = () => {
 ...
 const { products } = useContext(ProductContext); // Get products from context
  const navigate = useNavigate();
    ...
    const specialpricesProducts = products.filter(
      (product) => product.category === "specialprices"
    );
  ...
  return (
    <div>
         ...
         <div className='flex h-10 w-full p-1 mb-20 border-t-2 border-b-2'>
          <div className="text-center font-semibold mx-auto hidden md:block">{specialpricesProducts.length} PRODUCTS</div>
    ...
    <div className="flex p-8"> {/* Flex container for sidebar & products */}
     ...
     <hr/>
      ...
      {specialpricesProducts.map((product) => ( // Use specialprices Products instead of products
            <div 
              key={product.id} 
...
//     .then((data)=>{
//       const filteredProducts = data.filter((product) =>product.category === "specialprices" );
