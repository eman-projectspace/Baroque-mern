...
const seperatesProducts = products.filter(
  (product) => product.category === "seperates"
);

  return (
    <div>
           <h1 className='text-center mt-20 mb-20 text-4xl font-serif' >SEPERATES</h1>
         <div className='flex h-10 w-full p-1 mb-20 border-t-2 border-b-2'>
          <div className="text-center font-semibold mx-auto hidden md:block">{seperatesProducts.length} PRODUCTS</div>
    ...
    <div className="flex p-8"> {/* Flex container for sidebar & products */}
     ...
     <hr/>
      ...
      {seperatesProducts.map((product) => ( // Use sperates Products instead of products
            <div 
              key={product.id} 
...
//   //Fetch products fromm JSON

//   useEffect(()=>{
//     fetch("/products.json")
//     .then((response)=>response.json())
//     .then((data)=>{
//       const filteredProducts = data.filter((product) =>product.category === "seperates" );
